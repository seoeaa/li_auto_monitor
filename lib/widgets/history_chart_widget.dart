...
class _HistoryChartWidgetState extends State<HistoryChartWidget> {
  ...
  void initState() {
    super.initState();
    unawaited(HistoryService.init().catchError((Object error) {
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder<int>(
      ...
      builder: (context, value, child) {
        ...
        final history = HistoryService.segmentsFor(widget.host.host, startTime, now);
        final statistics = HistoryService.statistics(widget.host.host, startTime, now);
        final uptime = statistics.uptimePercent;
        final formatter = _selectedDays == 1 ? DateFormat('HH:mm') : DateFormat('dd.MM');
        return Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(widget.host.host, style: const TextStyle(color: AppTheme.textSecondary, fontSize: 12)),
            ...
            const SizedBox(height: 12),
            if (_error != null)
              Text(_error!, style: const TextStyle(color: AppTheme.statusUnknown))
            else ...[
              Text(
                uptime == null ? 'Недостаточно измерений' :
              ...
              const SizedBox(height: 12),
              Semantics(
                label: 'История доступности. Покрытие ${statistics.coveragePercent.toStringAsFixed(1)} процентов.',
                child: ClipRRect(
                  borderRadius: BorderRadius.circular(8),
                  child: SizedBox(
                    height: 56,
                    width: double.infinity,
                    child: CustomPaint(painter: UptimePainter(history: history, startTime: startTime, endTime: now)),
                  ),
                ),
              ),
              const SizedBox(height: 8),
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Text(formatter.format(startTime), style: const TextStyle(fontSize: 11)),
                Text(formatter.format(now), style: const TextStyle(fontSize: 11)),
              ]),
            ],
          ]),
        );
...
class UptimePainter extends CustomPainter {
  ...
  UptimePainter({required this.history, required this.startTime, required this.endTime});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final entry in history) {
      final left = (entry.timestamp.difference(startTime).inMilliseconds / totalMs).clamp(0.0, 1.0).toDouble() * size.width;
      final right = (entry.observedUntil.difference(startTime).inMilliseconds / totalMs).clamp(0.0, 1.0).toDouble() * size.width;
      ...
      canvas.drawRect(Rect.fromLTRB(left, 0, right, size.height), Paint()..color = color);
    }
    canvas.restore();
  }
  ...
  bool shouldRepaint(covariant UptimePainter oldDelegate) =>
      oldDelegate.history != history || oldDelegate.startTime != startTime || oldDelegate.endTime != endTime;
}
