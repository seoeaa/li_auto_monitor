...
DiagnosticResult dnsFailure() => DiagnosticResult(
  steps: const [
...
void main() {
  ...
  for (final code in [301, 302, 401, 403, 404, 405, 429, 500, 502, 503, 504]) {
    test(
      'HTTP $code is a visible remark, not a network outage or green health',
      () {
        final result = resultFor(code);
  ...
  test('unknown and empty groups are never green', () {
    ...
    expect(
      aggregateHostStates([HostState.unknown, HostState.unknown]),
      HostState.unknown,
    );
    expect(
      aggregateHostStates([HostState.online, HostState.unknown]),
      HostState.unknown,
    );
    expect(
      aggregateHostStates([HostState.online, HostState.online]),
      HostState.online,
    );
  });
  ...
  test('result replacement clears stale IP and downstream failure flags', () {
    ...
    expect(
      host.checkSteps.skip(1).every((step) => step.state == CheckState.skipped),
      isTrue,
    );
    host.dispose();
  });
  ...
  }) => HistoryService.saveObservations([
    HistoryEntry(
      host: 'api.invalid',
  ...
  test('one observation does not claim any uptime', () async {
    ...
    final stats = HistoryService.statistics(
      'api.invalid',
      base,
  ...
  test(
    'history ends at last measurement instead of extending through the night',
    () async {
      await sample(0, HostState.online);
  ...
  test(
    'long gaps and new sessions never bridge missing observations',
    () async {
      await sample(0, HostState.online);
      await sample(30, HostState.online);
  ...
  test('legacy samples migrate without invented continuity', () async {
    await File('${directory.path}/monitor_history.json').writeAsString(
      jsonEncode([
        {
          'host': 'api.invalid',
        ...
        {
          'host': 'api.invalid',
          'timestamp': base.add(const Duration(hours: 8)).toIso8601String(),
          'isOnline': true,
        },
      ]),
    );
    ...
    expect(
      HistoryService.calculateUptime(
        'api.invalid',
        base,
        base.add(const Duration(days: 1)),
      ),
  ...
  test(
    'persisted intervals survive reload with concurrent initialization',
    () async {
      ...
      await HistoryService.init();
      expect(
        HistoryService.getHistory('api.invalid').single.state,
        HostState.degraded,
      );
      expect(
  ...
  test('corrupt primary history recovers the last valid backup', () async {
    final entry = HistoryEntry(
      ...
      observedUntil: base.add(const Duration(seconds: 30)),
      sessionId: 's',
    );
    await File(
      '${directory.path}/monitor_history.v2.json',
    ).writeAsString('broken-json');
    await File('${directory.path}/monitor_history.v2.json.bak').writeAsString(
      jsonEncode({
        'version': 2,
    ...
    await HistoryService.init();
    expect(
      HistoryService.getHistory('api.invalid').single.observedUntil,
      entry.observedUntil,
    );
    await sample(60, HostState.down);
    await HistoryService.resetForTesting(
  ...
  test('refresh joins the same bounded worker pool', () async {
    ...
    final monitor = MonitorService(
      initialHosts: [
        for (var i = 0; i < 7; i++)
          HostStatus.unknown('APP', 'API $i', '$i.invalid'),
      ],
      probe: probe,
      controlHosts: [],
    );
  ...
  test(
    'failed control hosts do not override successful Li Auto TLS evidence',
    () async {
      final probe = FakeProbe(
        (host, token) async =>
            host == 'api.invalid' ? resultFor(503) : dnsFailure(),
      );
      final monitor = MonitorService(
        initialHosts: [HostStatus.unknown('APP', 'API', 'api.invalid')],
        probe: probe,
        controlHosts: ['control.invalid'],
      );
  ...
  test('optional hosts do not define the main verdict', () async {
    final monitor = MonitorService(
      ...
      probe: FakeProbe(
        (host, token) async =>
            host == 'api.invalid' ? resultFor(200) : dnsFailure(),
      ),
      controlHosts: [],
    );
    await monitor.refreshAllHosts();
  ...
  test(
    'dispose cancels the cycle and ignores a late non-cooperative result',
    () async {
