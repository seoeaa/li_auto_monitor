...
void main() {
  ...
  NetworkProbe probe({
    ...
    Duration stage = const Duration(seconds: 2),
  ...
  test(
    'one DNS lookup, selected IP, validated hostname and original Host header',
    () async {
      var lookups = 0;
      ...
      expect(
        result.steps.every((step) => step.state == CheckState.success),
        isTrue,
      );
  ...
  for (final code in [301, 401, 403, 404, 405, 429, 503]) {
    test(
      'real HTTP $code remains reachable with an explicit warning',
      () async {
        handler = (request) async {
  ...
  test(
    'untrusted certificate fails without any bad-certificate bypass',
    () async {
      final result = await probe(
  ...
  test('DNS failure skips all dependent stages', () async {
    final result = await probe(
      lookup: (_) async => throw const SocketException('No address'),
    ).check('localhost', CheckCancellation(), port: server.port);
    expect(result.steps[0].state, CheckState.failure);
    expect(
      result.steps.skip(1).every((step) => step.state == CheckState.skipped),
      isTrue,
    );
  ...
  test(
    'an unavailable IPv6 address falls back to IPv4 without resolving again',
    () async {
      var lookups = 0;
      final result = await probe(
        lookup: (_) async {
  ...
  test(
    'HTTP headers timeout is bounded while TCP and TLS stay successful',
    () async {
      handler = (_) async {};
      final watch = Stopwatch()..start();
      final result = await probe(
        stage: const Duration(milliseconds: 300),
      ).check('localhost', CheckCancellation(), port: server.port);
      expect(result.steps[1].state, CheckState.success);
  ...
  test(
    'late DNS completion cannot start a socket after cancellation',
    () async {
      final pending = Completer<List<InternetAddress>>();
      final token = CheckCancellation();
      final resultFuture = probe(
        lookup: (_) => pending.future,
      ).check('localhost', token, port: server.port);
      token.cancel();
  ...
  test('TLS handshake timeout closes the accepted TCP socket', () async {
    ...
    try {
      final result = await probe(
        stage: const Duration(milliseconds: 250),
